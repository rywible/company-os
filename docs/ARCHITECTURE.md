...
The inbox renders actual run snapshots rather than recomputing what a past reply might have seen. The library editor places optional context controls first, then the title and Markdown writing surface. It does not expose a subject-type picker. Inclusion, lifecycle, scope, and knowledge kind remain separate policy controls. Optional organization and revision history sit on the reading page. No synthetic subject pages or findings are seeded during migration. Human-created legacy knowledge becomes Unfiled subjects, machine-created notes become pending evidence, and document IDs and revisions are retained.
