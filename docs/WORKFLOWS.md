...
2. From a milestone’s optional **Execution details**, open a completed investigation and link its GitHub PR. `LinkPullRequest` verifies the current open PR and emits `WorkCompleted`. Agents do not invent a completion event or a PR URL from prose.
...
The planning automation defaults to two unfinished milestones, a four-hour cadence and four runs per UTC day. Completion opens capacity; the next due automation run can replenish it. Foreman is instructed to avoid duplicates and low-value busywork. Unanswered decisions never authorize work. Triage availability defaults to weekdays 09:00–17:00 America/Denver and does not gate worker execution.
...
Milestone planning is represented in the same automation list, with its cadence, run allowance, upcoming milestone limit, model and permissions. Its pause, run-now and delete actions follow the standard task lifecycle. Human milestone revisions happen through the milestone's Inbox conversation; Foreman returns a versioned revised proposal. Execution details remain available behind the optional DAG expansion for diagnosis.
