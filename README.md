...
Foreman proposes substantial assignments by role, establishing interfaces before parallel vertical streams. The optional dependency graph complements a readable assignment list. Roles expose names and capability descriptions to Foreman; provider, model and reasoning settings stay in Settings and are snapshotted when a run is queued. The initial roles are Architect, Implementer, Reviewer and Investigator.

Automatic planning checks every four hours, with at most four planning runs per UTC day and a target of two proposed, active or paused milestones. These controls live under Work → Automations. A full pipeline suppresses new planning; completed work clears the planning delay so the next scheduler tick can replenish it. Proposals always need explicit human approval. Run allowances include workers, reviews and failed-run retries; assignment review stops after three unsuccessful worker attempts.
