...
export function engineeringCommand(c: AgentConfiguration, dir: string, prompt: string, connector?: string) {
  ...
  return {
    command: ["muse", "exec", "--json", "--provider", "meta", "--preset", "native-basic", ...model,
      ...
      "--sandbox-network", "unrestricted", "--approval-mode", "never", "--user-input-auto-resolve",
...
export class SpriteEngineering {
  ...
  async run(runId: string, context: Context, configuration: AgentConfiguration, assigned: (worker: string) => void) {
    const checkout = context.implementation;
    ...
    const prompt = `You are ${context.role?.name || "the implementation worker"}, autonomously carrying out an approved engineering assignment.
...
${renderBriefing(context)}`;
    ...
    const payload = {
      ...
      prompt, schema, timeoutMs, provider: configuration.provider, gateway: this.gateway(),
      ...engineeringCommand(configuration, dir, prompt, connector), worker: checkout.worker,
    };
